...
def fetch_news_headlines(company_name):
    ...
    try:
        ...
        # Search for news about the company with financial terms filter
        query = f'"{company_name}" stock OR earnings OR revenue OR analyst OR acquisition OR guidance OR shares'
        ...
        if response['status'] == 'ok' and response['totalResults'] > 0:
            ...
            for article in response['articles']:
                ...
                if url not in seen_urls and not any(kw in title for kw in junk_keywords):
                    seen_urls.add(url)
...
def generate_html_tearsheet(ticker, company_name, financial_data, headlines, ai_summary, market_indices, ytd_data, quarterly_data, analyst_data, enhanced_stats):
...
<body>
    <div class="header">
        <div class="header-left">
            <h1>
                {f'<img src="https://logo.clearbit.com/{domain}" height="36" style="vertical-align: middle; margin-right: 10px;" onerror="this.style.display=\'none\'">' if domain else ''}{company_name} <span class="ticker-box">{ticker.upper()}</span>
            </h1>
            <div class="subtitle">{sector} · {industry} · {format_large_number(financial_data.get('market_cap', 'N/A'))}</div>
        </div>
        <div class="header-center">
            <input type="text" id="searchInput" placeholder="Search ticker" onkeypress="handleSearchKeypress(event)">
        </div>
        <div class="header-right">
            <div class="price-large">${current_price}</div>
            <div class="change-pill" style="background-color: {daily_color}; color: #000000;">{daily_change_pct}</div>
        </div>
    </div>
    ...
    <div class="market-indices">
        ...
        <div class="index-card">
            <div class="index-name">Avg Volume</div>
            <div class="index-value">{format_large_number(enhanced_stats.get('avg_volume', 'N/A'))}</div>
        </div>
        <div class="index-card">
            <div class="index-name">Dividend Yield</div>
            <div class="index-value">{format_percentage(enhanced_stats.get('dividend_yield', 'N/A'))}</div>
        </div>
    </div>
            ...
            <div class="card">
                ...
                <div class="stats-grid">
                    <div class="stat-item">
                        ...
                        <div class="stat-value">{format_large_number(financial_data.get('market_cap', 'N/A'))}</div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-label">P/E Ratio (TTM)</div>
                        <div class="stat-value">{format_ratio(financial_data.get('pe_ratio', 'N/A'))}</div>
                    </div>
                    <div class="stat-item">
...
def get_financial_data(ticker):
    ...
    try:
        ...
        ai_summary = ""
        if company_name != 'N/A':
            ai_summary = get_ai_analyst_summary(company_name, financial_data, headlines)
...
@app.route('/tearsheet')
def tearsheet():
    """Serve the tearsheet page for a given ticker"""
    ...
    try:
        ...
        return f"Error generating tearsheet: {e}", 500
